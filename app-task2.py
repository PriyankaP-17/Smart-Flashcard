...
def get_mixed_flashcards_by_student():
    student_id = request.args.get("student_id")
    ...
    # Group flashcards by subject for that student
    subject_groups = (
        db.session.query(Flashcard.subject)
        .filter_by(student_id=student_id)
        .group_by(Flashcard.subject)
        .all()
    )

    subject_names = [s[0] for s in subject_groups]

    per_subject = max(1, limit // len(subject_names)) if subject_names else limit
    ...
    for subject in subject_names:
        flashcards = (
            Flashcard.query
            .filter_by(student_id=student_id, subject=subject)
            .order_by(func.random())
            .limit(per_subject)
            .all()
        )
        results.extend(flashcards)
